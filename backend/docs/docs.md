...
* Picking Positions always have a 'z' coordinate equal to the configured box height.

## Worth Noting

* No tool is attached to the arm therefore TCP is equivalent to the flange head.

# Palletizer API Documentation
...
```json id="c1q9aa"
{
  ...
  "box_depth_mm": 100,
  "box_height_mm": 120,
  "pallet_origin_x_mm": 100,
  "pallet_origin_y_mm": 100,
...
### 3. Provided API Docs and Tester
...
To run the palletizer sequence, open the docs link above and use the following endpoints:
1. Configure: Will set the palletizer grid with the default layout (lines up with simulated detections file)
2. Start: Will commence the palletizing operations.

## Future Improvements

* Allow for 3D pallet grid configuration
* Connect unit test suite to Github Actions or equivalent
* Spawn Async task for full time graphing and logging of tcp position
* Consider joint space operations to avoid singularities and other such issues.

## Useful Documents
* User Manual (Downloaded along side this file)
* RTDE Control API https://sdurobotics.gitlab.io/ur_rtde/api/api.html#rtde-control-api
* RTDE Receive API https://sdurobotics.gitlab.io/ur_rtde/api/api.html#rtde-receive-interface-api
* Vention State Machine Docs https://docs.vention.io/docs/state-machine